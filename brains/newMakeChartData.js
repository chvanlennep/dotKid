...
const makeChartData = () => {
  const generateMeasurementArray = (
    measurementType,
    sex,
    z,
    startInDays,
    ...
    const returnArray = [];
    for (let i = startInDays; i <= endInDays; i++) {
      const lms = isChild
        ? generateLMSChild(measurementType, sex, i)
        : generateLMSPreterm(measurementType, sex, i);
      const measurement = computeChildMeasurementForGraph(z, lms);
      returnArray.push(measurement);
    }
    return returnArray;
  };
};
